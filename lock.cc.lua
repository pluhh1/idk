...
NotifyLib.prompt('Notification', 'Loaded Pluh\'s Lock!', 5)
function customNotify(title, text, duration)
    ...
    -- TouchBlocker properties
    touchBlocker.Parent = notification
    touchBlocker.BackgroundTransparency = 1.0
...
TextButton.MouseButton1Click:Connect(function()
    ...
    if state then
        ...
        Plr = FindNearestEnemy()
        if Plr then
            customNotify("Locked onto:", tostring(Plr.Parent.Name), 2)
        else
            customNotify("No target found", "", 2)
        end
    else
        TextButton.Text = "Lock • Off"
        enabled = false
        Plr = nil
        customNotify("Unlocked!", "", 2)
    end
end)
